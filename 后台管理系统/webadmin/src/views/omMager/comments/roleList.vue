        ...
        <el-form label-width="120px">
          <el-row class="el-row-el ele-tree ml" v-if="data2.length">
            ...
            <div style="text-align: left;">
              <el-checkbox
                :indeterminate="isTreeAll"
                v-model="checkAllTree"
                @change="TreeAllChange"
              >全选</el-checkbox>
            </div>
            <el-tree
              :data="data2"
...
export default {
  ...
  methods: {
    ...
    TreeChange(data, checked, indeterminate) {
      let that = this;
      let nowLength = that.$refs.tree
        .getCheckedKeys()
        .concat(that.$refs.tree.getHalfCheckedKeys()).length;
      if (nowLength === 0) {
    ...
    async getList() {
      ...
      try {
        ...
        if (response.data.code === 1) {
          console.log(response.data, "权限管理数据");
