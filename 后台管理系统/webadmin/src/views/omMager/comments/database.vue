  ...
  <el-drawer
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm"  v-loading='loading'>
      <el-form-item
        ...
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.name" placeholder="请输入数据库名称"></el-input>
      </el-form-item>
      <el-form-item
        prop="nodeId"
        label="节点名称:"
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-select clearable v-model="ruleForm.nodeId" :disabled='ProhibitFromm' @change="selectGet" placeholder="请选择节点名称">
          <el-option
       ...
       <el-form-item
        ...
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-select clearable v-model="ruleForm.type" :disabled='ProhibitFromm'  placeholder="请选择数据库类型">
          <el-option
      ...
      <el-form-item
        ...
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFrom' autocomplete="off" v-model="ruleForm.username" placeholder="请输入用户名"></el-input>
      </el-form-item>
      <el-form-item
        label="密码:"
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFrom' autocomplete="off" v-model="ruleForm.password" placeholder="请输入密码"></el-input>
      </el-form-item>
      <el-form-item
        label="备注:"
        :label-width="this.formLabelWidth"
...
import { insertDatabase, updateDatabase, findIdDatabase, findAllDatabaseNodes } from '@/api/omManger/comments'
export default {
  components: {
    // 注册组件
    // editor: editor
  ...
  data () {
    return {
      ...
      obj: {},
      options: [],
      optionssss:[{value:'Mysql',lable:'Mysql'},{value:'Postgresql',lable:'Postgresql'}],
      rules: {
        name: [{
          required: true,
          message: '请输入数据库名称',
  ...
      });
    ...
    async initFormDetail (value) {
      ...
      try {
        ...
        that.loading = true
        const response = await findIdDatabase(parameter)
        that.loading = false
        that.ruleForm = response.data.data
        if (this.modalObjjf === '详情') {
          that.ProhibitFrom = true
          // 禁用编辑器
        } else if (this.modalObjjf === '编辑') {
    ...
    async add(data) {
      ...
      that.loading = true
      const response = await insertDatabase(da)
      that.loading = false
    ...
    async findNodes(data) {
      const that = this
      that.loading = true
      const response = await findAllDatabaseNodes()
      that.loading = false
      if (response.data.code === 1) {
        that.options = []
