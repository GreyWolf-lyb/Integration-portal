  ...
  <el-main>
     <el-col :span="24">
      <el-form :inline="true" class='el-InputForm'>
        <el-form-item>
          <el-input clearable size="medium" placeholder="请输入模型名称" prefix-icon="el-icon-search" v-model="SearchItem.name">
          </el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-col :span='24'>
     ...
     <drawer :modal-obj='modalObj' ref="callDetail" @Reload="submitLoading"></drawer>
  </el-main>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],
      tableHeader: [{
        id: false, type: '', label: '模型名称', list: 'name'
      }, {
    ...
    async fetchData() {
      ...
      try {
        ...
        res.data.data.content.map(function(v, k){
          switch(v.type){
            case 'independentService': v.type = "独立服务模型"; 
            break; 
            case 'compositeService': v.type = "复合服务模型"; 
            break; 
            case 'choreographyServices': v.type="编排服务模型"; 
            break; 
            case 'rules': v.type="规则模型"; 
            break; 
            case 'standardSpecification': v.type="标准规范模型"; 
            break; 
            case 'pageComponents': v.type="页面组件模型"; 
            break; 
            case 'xmlStructural': v.type="XML结构模型"; 
            break;
            case 'modeMatching': v.type="模式匹配模型"; 
            break; 
            case 'modeRecognition': v.type="模式识别模型"; 
            break;
            case 'other': v.type="其它模型"; 
            break; 
          }
        })
